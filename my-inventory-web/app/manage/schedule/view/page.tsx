...
// Context & Services
// ✅ เปลี่ยนตามที่คุณกรแก้ไขไว้ครับ
...
import { 
  ...
  deleteUserActivity, 
  generateLineReport,
  generateAIPromptJSON, // ✨ ดึงฟังก์ชันใหม่มาใช้
  getAllProjects,
  fetchDictionary,
...
export default function SchedulePage() {
  ...
  // ✨ State สำหรับ Modal รายงาน
  const [isReportOpen, setIsReportOpen] = useState(false);
  const [reportText, setReportText] = useState("");
  const [jsonPromptText, setJsonPromptText] = useState("");
  const [activeTab, setActiveTab] = useState<'line' | 'json'>('line');
  ...
  const copyToClipboard = () => { 
    ...
    alert("คัดลอกข้อมูลแล้ว นำไปวางต่อได้เลย!"); 
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      {/* ✨ Modal สรุปรายงาน (มี Tab เลือกแบบ ข้อความ กับ แบบ JSON) */}
      {isReportOpen && (
        <div className="fixed inset-0 bg-black/60 z-[1000] flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden flex flex-col h-[80vh] animate-in zoom-in-95">
            <div className="p-5 bg-emerald-50 border-b border-emerald-100 flex justify-between items-center shrink-0">
            ...
            {/* โซนเลือก Tab */}
            <div className="flex bg-slate-100 p-2 m-4 rounded-xl shrink-0">
               <button 
                  onClick={() => setActiveTab('line')}
                  className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${activeTab === 'line' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
               >
                  💬 ข้อความ LINE
               </button>
               <button 
                  onClick={() => setActiveTab('json')}
                  className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${activeTab === 'json' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
               >
                  🤖 JSON (สำหรับ AI)
               </button>
            </div>
            ...
            <div className="px-4 pb-4 flex-1 overflow-hidden">
               <textarea 
                  className="w-full h-full p-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono leading-relaxed resize-none text-slate-800 focus:outline-none focus:border-emerald-400" 
            ...
            <div className="p-4 border-t border-slate-100 flex gap-3 shrink-0 bg-white">
               ...
               <button onClick={copyToClipboard} className={`flex-1 text-white py-4 rounded-2xl font-black uppercase text-xs shadow-lg flex justify-center items-center gap-2 transition-colors tracking-widest ${activeTab === 'line' ? 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-200' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-200'}`}>
                  <Copy size={16} /> {activeTab === 'line' ? 'คัดลอกส่ง LINE' : 'คัดลอกให้ AI'}
               </button>
            </div>
          </div>
        </div>
