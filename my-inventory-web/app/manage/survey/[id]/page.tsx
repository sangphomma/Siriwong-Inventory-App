        ...
        <div className="fixed inset-0 bg-black/60 z-[1000] flex items-end sm:items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl animate-in slide-in-from-bottom-10 h-[80vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveMember} className="space-y-4 font-sans px-1">
              ...
              <div>
                <label className="text-[10px] font-black text-slate-400 ml-1 mb-2 block uppercase tracking-widest">หน้าที่ (Role)</label>
                <input className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-200 text-sm font-black outline-none" value={memberForm.role} onChange={e => setMemberForm({...memberForm, role: e.target.value})} required placeholder="เช่น คนวัดพื้นที่" />
              </div>
